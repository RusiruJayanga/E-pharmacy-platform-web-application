...
const Appointment_doctor = () => {
    ...
    const fetchAppointment = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/appointment-lab/get/${appointmentId}`
        );
  ...
  const handleApprove = async (e) => {
    ...
    try {
      await axios.post(
        `http://localhost:5000/api/appointment-lab/approve/${appointmentId}`,
        {
  ...
  const handleReject = async (e) => {
    ...
    try {
      await axios.post(
        `http://localhost:5000/api/appointment-lab/reject/${appointmentId}`
      );
  ...
  return (
    <div>
      ...
      <div className="appointment-info-image">
        <h5>Prescription</h5>
        <img src={appointment.image} alt={appointment._id} />
      </div>
