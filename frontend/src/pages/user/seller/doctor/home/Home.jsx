...
const Home = () => {
  ...
  const fetchAllData = async () => {
    ...
    const token = JSON.parse(localStorage.getItem("doctorToken"))?.token;
    if (!token) {
    ...
    try {
      ...
      const [newRes, endRes, rejectRes, statsRes] = await Promise.all([
        axios.get("http://localhost:5000/api/doctor/appointments/new", {
          headers: authHeader,
        }),
        axios.get("http://localhost:5000/api/doctor/appointments/end", {
          headers: authHeader,
        }),
        axios.get("http://localhost:5000/api/doctor/appointments/rejected", {
          headers: authHeader,
        }),
  ...
  const handleCardClick = (appointmentId) => {
    navigate(`/Appointment_doctor`, { state: { appointmentId } });
  };
